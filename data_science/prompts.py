...
def return_instructions_root() -> str:
    ...
    - If user wants to calculate risk, route to the risk_agent and provide the input data in the required format of a single claim only.
    - If data provided by user is not sufficient for the task, ask the user for more details.
    - All queries with risk agent must be in the format "{
  "input_data": {
    "age": 40,
    "income": 85900,
    "vehicle_age": 3,
    ...
    <TASK>
        ...
        # 4b. **Risk Agent Tool (`call_risk_agent` - if applicable):**  If the user specifically asks (!) for risk analysis, use this tool. Make sure to provide a proper query to it to fulfill the task, along with the dataset and project ID, and context. Must be of a single claim data only.
    ...
    - a risk agent (`call_risk_agent`), if necessary.
    ...
    #   B. You pass the project and dataset ID.
    #   C. You pass any additional context.
    #   * **Risk Agent `call_risk_agent`:** Query the Risk Agent if the user asks for it. Ensure that:
    #   A. You provide the fitting query.
    #   B. You pass the single claim details in the correct format.
    #   C. You pass any additional context.
